...
#ifdef PSET_SCRIPT
  struct trailstate_s
      *trailstate; // spike -- managed by the particle system, so we don't loose
                   // our position and spawn the wrong number of particles, and
                   // we can track beams etc
  struct trailstate_s
...
void R_RenderView(bool use_tasks, task_handle_t begin_rendering_task,
                  task_handle_t setup_frame_task,
                  task_handle_t draw_done_task); // must set r_refdef first
