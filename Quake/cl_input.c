  ...
  if (cmd) {
    ...
    if (cl.protocol_pext2 & PEXT2_PREDINFO) {
      MSG_WriteShort(&buf, cl.movemessages &
                               0xffff); // server will ack this once it has been
                                        // applied to the player's entity state
      MSG_WriteFloat(&buf,
                     cmd->servertime); // so server can get cmd timing (pings
                                       // will be calculated by entframe acks).
    } else
      MSG_WriteFloat(&buf, cl.mtime[0]); // so server can get ping times

    for (i = 0; i < 3; i++)
      // johnfitz -- 16-bit angles for PROTOCOL_FITZQUAKE
      // spike -- nq+bjp3 use 8bit angles. all other supported protocols use
      // 16bit ones. spike -- proquake servers bump client->server angles up to
      // at least 16bit. this is safe because it only happens when both
      // client+server advertise it, and because it never actually gets recorded
      // into demos anyway. spike -- predinfo also always means 16bit angles,
      // even if for some reason the server doesn't advertise proquake (like
      // dp).
      if (cl.protocol == PROTOCOL_NETQUAKE &&
          !NET_QSocketGetProQuakeAngleHack(cls.netcon) &&
