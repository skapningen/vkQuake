...
hash_map_t *
HashMap_CreateImpl(const uint32_t key_size, const uint32_t value_size,
...
bool HashMap_InsertImpl(hash_map_t *map, const uint32_t key_size,
                        const uint32_t value_size, const void *const key,
                        const void *const value);
bool HashMap_EraseImpl(hash_map_t *map, const uint32_t key_size,
...
#define HashMap_Create(key_type, value_type, hasher, comp)                     \
  HashMap_CreateImpl(sizeof(key_type), sizeof(value_type), hasher, comp)
#define HashMap_Insert(map, key, value)                                        \
  HashMap_InsertImpl(map, sizeof(*key), sizeof(*value), key, value)
#define HashMap_Erase(map, key) HashMap_EraseImpl(map, sizeof(*key), key)
#define HashMap_Lookup(type, map, key)                                         \
  ((type *)HashMap_LookupImpl(map, sizeof(*key), key))
#define HashMap_GetKey(type, map, index)                                       \
  ((type *)HashMap_GetKeyImpl(map, index))
#define HashMap_GetValue(type, map, index)                                     \
  ((type *)HashMap_GetValueImpl(map, index))
