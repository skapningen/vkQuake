...
static qboolean CL_LerpEntity(entity_t *ent, vec3_t org, vec3_t ang,
                              float frac) {
  float f, d, a;
  int j;
  vec3_t delta;
...
static qboolean CL_AttachEntity(entity_t *ent, float frac) {
  ...
  while (1) {
    ...
    // what should be displayed somewhere far removed from the code that deals
    // with timing so we have absolutely no way to get a reliable origin in the
    // meantime, r_lerpmove 0; r_lerpmodels 0 you might be able to work around
    // it by setting the attached entity to movetype_step to match the attachee,
    // and to avoid EF_MUZZLEFLASH. personally I'm just going to call it a
    // quakespasm bug that I cba to fix.
...
void CL_RelinkEntities(void) {
  ...
  if (cl_numvisedicts + 256 > cl_maxvisedicts) {
    ...
    cl_visedicts =
        Mem_Realloc(cl_visedicts, sizeof(*cl_visedicts) * cl_maxvisedicts);
    cl_visedicts_alpha = Mem_Realloc(
        cl_visedicts_alpha, sizeof(*cl_visedicts_alpha) * cl_maxvisedicts);
  }
  cl_numvisedicts = 0;
  ...
  for (i = 1; i < cl.num_entities; i++, ent++) {
    ...
    if (cl.paused)
      ;
    else if (ent->netstate.traileffectnum > 0 &&
             ent->netstate.traileffectnum < MAX_PARTICLETYPES) {
      vec3_t axis[3];
      AngleVectors(ent->angles, axis[0], axis[1], axis[2]);
      PScript_ParticleTrail(
          oldorg, ent->origin,
          cl.particle_precache[ent->netstate.traileffectnum].index, frametime,
          i, axis, &ent->trailstate);
    } else if (ent->model->traileffect >= 0) {
      vec3_t axis[3];
      AngleVectors(ent->angles, axis[0], axis[1], axis[2]);
      PScript_ParticleTrail(oldorg, ent->origin, ent->model->traileffect,
                            frametime, i, axis, &ent->trailstate);
    } else
#else
...
void CL_SendCmd(void) {
  ...
  cmd.forwardmove +=
      cl.pendingcmd.forwardmove + cl.pendingcmd.forwardmove_accumulator;
...
static void CL_ServerExtension_FullServerinfo_f(void) {
  ...
  memcpy(cl.serverinfo, newserverinfo,
         sizeof(cl.serverinfo)); // just replace it
}
static void CL_ServerExtension_ServerinfoUpdate_f(void) {
  ...
  Info_SetKey(cl.serverinfo, sizeof(cl.serverinfo), newserverkey,
              newservervalue);
}
...
static void CL_ServerExtension_FullUserinfo_f(void) {
  ...
  if (slot < cl.maxclients) {
    ...
    strncpy(sb->userinfo, newserverinfo,
            sizeof(sb->userinfo) - 1); // just replace it
...
static void CL_ServerExtension_UserinfoUpdate_f(void) {
  ...
  if (slot < cl.maxclients) {
    scoreboard_t *sb = &cl.scores[slot];
    Info_SetKey(sb->userinfo, sizeof(sb->userinfo), newserverkey,
                newservervalue);
    CL_UserinfoChanged(sb);
  }
}
...
void CL_Init(void) {
  ...
  Cmd_AddCommand_ServerCommand("fullserverinfo",
