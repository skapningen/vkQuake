...
float M_GetSliderPos(float low, float high, float current, bool backward,
                     bool mouse, float clamped_mouse, int dir, float step,
                     float snap_start);
...
void M_Mouse_UpdateListCursor(int *cursor, int left, int right, int top,
                              int item_height, int num_items,
                              int scroll_offset);
void M_Mouse_UpdateCursor(int *cursor, int left, int right, int top,
                          int item_height, int index);

void M_Menu_Video_f(void);
void M_Video_Draw(cb_context_t *cbx);
void M_Video_Key(int key);
bool M_HandleScrollBarKeys(const int key, int *cursor, int *first_drawn,
                           const int num_total, const int max_on_screen);
