...
static void dump_f(void) {
  ...
  q_snprintf(name, sizeof(name), "%s/%s", com_gamedir,
             Cmd_Argc() >= 2 ? Cmd_Argv(1) : "condump.txt");
  COM_CreatePath(name);
  f = fopen(name, "w");
  if (!f) {
...
================
*/
static void message_mode_f(void) {
...
================
*/
static void message_mode2_f(void) {
  if (cls.state != ca_connected || cls.demoplayback)
    return;
  chat_team = true;
  key_dest = key_message;
...
void init(void) {
  ...
  Cmd_AddCommand("toggleconsole", toggle_console_f);
  Cmd_AddCommand("messagemode", message_mode_f);
  Cmd_AddCommand("messagemode2", message_mode2_f);
  Cmd_AddCommand("clear", clear_f);
  Cmd_AddCommand("condump", dump_f); // johnfitz
...
void DWarning(const char *fmt, ...) {
  ...
  if (developer.value >=
      2) { // don't confuse non-developers with techie stuff...
...
void Dprintf(const char *fmt, ...) {
  ...
  safe_printf("%s", msg); // johnfitz -- was Con_Printf
}
...
qboolean is_redirected(void) { return !!con_redirect_flush; }
void redirect(void (*flush)(const char *)) {
  if (con_redirect_flush)
...
void tab_complete(void) {
    ...
    if (!strncmp(key_lines[edit_line] + 1, command_name,
      ...
      *c = '\0';
      ...
      key_linepos = c - key_lines[edit_line] +
                    strlen(matched_map); // set new cursor position
  ...
  *c = '\0';             // now copy all of this into edit line
  q_strlcat(key_lines[edit_line], partial, MAXCMDLINE);
  key_linepos =
      c - key_lines[edit_line] + strlen(match); // set new cursor position
  if (key_linepos >= MAXCMDLINE)
    key_linepos = MAXCMDLINE - 1;
...
void draw_input(cb_context_t *cbx) {
  int i, ofs;
  ...
  for (i = 0; key_lines[edit_line][i + ofs] && i < con_linewidth; i++)
    Draw_Character(cbx, (i + 1) << 3, vid.conheight - 16,
                   key_lines[edit_line][i + ofs]);

  // johnfitz -- new cursor handling
  if (!((int)((realtime - key_blinktime) * con_cursorspeed) & 1)) {
    i = key_linepos - ofs;
    Draw_Pic(cbx, (i + 1) << 3, vid.conheight - 16,
             key_insert ? pic_ins : pic_ovr, 1.0f, false);
  }
}
...
================
*/
void draw_console(cb_context_t *cbx, int lines, qboolean drawinput) {
  int i, x, y, j, sb, rows;
  const char *text;
  char ver[32];
  ...
  for (x = 0; x < (int)strlen(ver); x++)
    Draw_Character(cbx, (con_linewidth - strlen(ver) + x + 2) << 3, y,
                   ver[x] /*+ 128*/);
}
...
==================
*/
void notify_box(const char *text) {
  double t1, t2;
  int lastkey, lastchar;

  // during startup for sound / cd warnings
  printf("\n\n%s", quakebar(40)); // johnfitz
  printf("%s", text);
  printf("Press a key.\n");
  printf("%s", quakebar(40)); // johnfitz
