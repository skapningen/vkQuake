...
typedef struct {
  ...
  int type;
  unsigned int
      hitcontents; // content types to impact upon... (1<<-CONTENTS_FOO) bitmask
  edict_t *passedict;
} moveclip_t;
...
areanode_t *SV_CreateAreaNode(int depth, vec3_t mins, vec3_t maxs) {
  ...
  if (pr_checkextension.value
          ? depth == MAX_AREA_DEPTH || size[anode->axis] < 500
...
void SV_TouchLinks(edict_t *ent) {
  ...
  listcount = 0;
  SV_AreaTriggerEdicts(ent, qcvm->areanodes, list, &listcount,
                       qcvm->num_edicts);
...
void SV_LinkEdict(edict_t *ent, qboolean touch_triggers) {
  ...
  // set the abs box
  if (ent->v.solid == SOLID_BSP && pr_checkextension.value &&
      IsOriginWithinMinMax(ent->v.origin, ent->v.mins, ent->v.maxs) &&
      !IsAxisAlignedDeg(
          ent->v.angles)) { // expand for rotation the lame way. hopefully
                            // there's an origin brush in there.
    int i;
    float v1, v2;
    vec3_t max;
    // q2 method
...
int SV_PointContentsAllBsps(vec3_t p, edict_t *forent) {
  trace_t trace = SV_Move(p, vec3_origin, vec3_origin, p,
...
reenter:
  ...
  if (t1 >= 0 && t2 >= 0) {
    // return Q1BSP_RecursiveHullTrace (hull, node->children[0], p1f, p2f, p1,
    // p2, trace);
    num = node->children[0];
    goto reenter;
  }
  if (t1 < 0 && t2 < 0) {
    // return Q1BSP_RecursiveHullTrace (hull, node->children[1], p1f, p2f, p1,
    // p2, trace);
    num = node->children[1];
    goto reenter;
  }
  ...
  rht = Q1BSP_RecursiveHullTrace(ctx, node->children[side], p1f, midf, p1, mid,
                                 trace);
  if (rht != rht_empty && !trace->allsolid)
    return rht;
  rht = Q1BSP_RecursiveHullTrace(ctx, node->children[side ^ 1], midf, p2f, mid,
                                 p2, trace);
  if (rht != rht_solid)
    return rht;
...
qboolean SV_SlowRecursiveHullCheck(hull_t *hull, int num, float p1f, float p2f,
...
#if 1
  if (t1 >= 0 && t2 >= 0)
    return SV_SlowRecursiveHullCheck(hull, node->children[0], p1f, p2f, p1, p2,
                                     trace);
  if (t1 < 0 && t2 < 0)
    return SV_SlowRecursiveHullCheck(hull, node->children[1], p1f, p2f, p1, p2,
                                     trace);
#else
  ...
  // move up to the node
  if (!SV_SlowRecursiveHullCheck(hull, node->children[side], p1f, midf, p1, mid,
                                 trace))
    return false;
  ...
  if (SV_HullPointContents(hull, node->children[side ^ 1], mid) !=
      CONTENTS_SOLID)
    // go past the node
    return SV_SlowRecursiveHullCheck(hull, node->children[side ^ 1], midf, p2f,
                                     mid, p2, trace);
...
==================
*/
qboolean SV_RecursiveHullCheck(hull_t *hull, vec3_t p1, vec3_t p2,
                               trace_t *trace, unsigned int hitcontents) {
  if (!pr_checkextension.value)
    return SV_SlowRecursiveHullCheck(hull, hull->firstclipnode, 0, 1, p1, p2,
                                     trace);
    ...
    return Q1BSP_RecursiveHullTrace(&ctx, hull->firstclipnode, 0, 1, p1, p2,
                                    trace) != rht_impact;
  }
}
...
trace_t SV_ClipMoveToEntity(edict_t *ent, vec3_t start, vec3_t mins,
                            vec3_t maxs, vec3_t end, unsigned int hitcontents) {
  trace_t trace;
  vec3_t offset;
  ...
  if (ent->v.solid == SOLID_BSP && pr_checkextension.value &&
      ...
      qcvm->edicts !=
          ent) // don't rotate the world entity's collisions (its not networked,
               // and some maps are buggy, resulting in screwed collisions)
  {
  ...
  for (l = node->solid_edicts.next; l != &node->solid_edicts; l = next) {
    ...
    if (touch->v.skin < 0) {
      ...
      if ((int)touch->v.flags & FL_MONSTER)
        trace =
            SV_ClipMoveToEntity(touch, clip->start, clip->mins2, clip->maxs2,
                                clip->end, ~(1u << -CONTENTS_EMPTY));
      else
      ...
      if ((int)touch->v.flags & FL_MONSTER)
        trace = SV_ClipMoveToEntity(touch, clip->start, clip->mins2,
                                    clip->maxs2, clip->end, clip->hitcontents);
      else
        trace = SV_ClipMoveToEntity(touch, clip->start, clip->mins, clip->maxs,
                                    clip->end, clip->hitcontents);
    }
...
static void World_ClipToNetwork(moveclip_t *clip) {
  ...
  for (i = 1, touch = cl.entities + 1; i < cl.num_entities; i++, touch++) {
    ...
    if (clip->type == MOVE_NOMONSTERS &&
        touch->netstate.solidsize != ES_SOLID_BSP)
      continue;
    ...
    */
    ...
    /*		if (clip->passedict)
                    {
                            if (PROG_TO_EDICT(touch->v.owner) ==
       clip->passedict) continue;	// don't clip against own missiles if
       (PROG_TO_EDICT(clip->passedict->v.owner) == touch) continue;	// don't
       clip against owner
                    }
    */
    ...
    //:clip->maxs, clip->end);
    {
      ...
      {
        vec3_t size;
        if (touch->netstate.solidsize == ES_SOLID_BSP && touch->model &&
            touch->model->type ==
                mod_brush) { // explicit hulls in the BSP model
      ...
      if (touch->netstate.solidsize == ES_SOLID_BSP &&
          (touch->angles[0] || touch->angles[1] || touch->angles[2]) &&
          pr_checkextension.value) // don't rotate the world entity's collisions
                                   // (its not networked, and some maps are
                                   // buggy, resulting in screwed collisions)
      {
#define DotProductTranspose(v, m, a)                                           \
  ((v)[0] * (m)[0][a] + (v)[1] * (m)[1][a] + (v)[2] * (m)[2][a])
        vec3_t axis[3], start_r, end_r, tmp;
