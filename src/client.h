...
typedef struct {
  ...
  usercmd_t movecmds[64]; // ringbuffer of previous movement commands (journal
                          // for prediction)
...
void CL_UpdateBeam(struct qmodel_s *m, const char *trailname,
                   const char *impactname, int ent, float *start, float *end);
