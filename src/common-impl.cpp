  ...
  while (1) {
    ...
    if (c1 == c2f) {
      for (i = 1;; i++) {
        ...
        if (!c2)
          return (
              char *)haystack; // end of needle means we found a complete match
        if (!c1) // end of haystack means we can't possibly find needle in it
                 // any more
          return NULL;
...
int wildcmp(const char *wild,
            const char *string) { // case-insensitive string compare with
                                  // wildcards. returns true for a match.
  while (*string) {
    if (*wild == '*') {
      if (*string == '/' || *string == '\\') {
        //* terminates if we get a match on the char following it, or if its a
        //\ or / char
        wild++;
        continue;
      }
...
void Info_RemoveKey(char *info,
                    const char *key) { // only shrinks, so no need for max size.
...
void Info_SetKey(char *info, size_t infosize, const char *key,
                 const char *val) {
  size_t keylen = strlen(key);
...
static void COM_SetupNullState(void) {
  ...
  nullentitystate.alpha =
      ENTALPHA_DEFAULT; // fte has 255 by default, with 0 for invisible. fitz
                        // uses 1 for invisible, 0 default, and 255=full alpha
...
void COM_Init(void) {
  ...
  PDP_ORDER: 34 12 78 56
             N  U  X  I
  */
  if (bytes[0] != 0x78 || bytes[1] != 0x56 || bytes[2] != 0x34 ||
      bytes[3] != 0x12)
...
static int COM_FindFile(const char *filename, int *handle, FILE **file,
  ...
  pack_t *pak;
  int i;
  bool is_config = !q_strcasecmp(filename, "config.cfg"), found = false;
  ...
  for (search = com_searchpaths; search; search = search->next) {
    ...
    } else /* check a file in the directory tree */
    {
      ...
      if (is_config) {
        q_snprintf(netpath, sizeof(netpath), "%s/" CONFIG_NAME,
                   search->filename);
...
_add_path:
  ...
  for (i = 0;; i++) {
    ...
    if ((i == 0) && (path_id == 1) && !fitzmode) {
      size_t vkquake_pak_size_compressed = vkquake_pak_size,
             vkquake_pak_size_extracted = vkquake_pak_decompressed_size;
      if (!vkquake_pak_extracted) {
        tinfl_decompressor inflator;
        tinfl_init(&inflator);
        vkquake_pak_extracted = Mem_Alloc(vkquake_pak_size_extracted);
        if (TINFL_STATUS_DONE !=
            tinfl_decompress(&inflator, vkquake_pak,
                             &vkquake_pak_size_compressed,
                             vkquake_pak_extracted, vkquake_pak_extracted,
                             &vkquake_pak_size_extracted,
                             TINFL_FLAG_USING_NON_WRAPPING_OUTPUT_BUF))
          Sys_Error("Error extracting embedded pack");
      }
      bool pak0_modified = com_modified;
      Sys_MemFileOpenRead(vkquake_pak_extracted, vkquake_pak_size_extracted,
                          &packhandle);
...
bool COM_ModForbiddenChars(const char *p) {
  return !*p || !strcmp(p, ".") || strstr(p, "..") || strstr(p, "/") ||
...
void LOC_LoadFile(const char *file) {
    ...
    if (line[0] == '/') {
      ...
      *key_end = 0;
      ...
      if (localization.numentries == localization.maxnumentries) {
        ...
        localization.entries = (locentry_t *)Mem_Realloc(
  ...
  localization.indices = (unsigned *)Mem_Realloc(
