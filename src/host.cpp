...
void Host_FindMaxClients(void) {
  ...
  svs.clients =
      (struct client_s *)Mem_Alloc(svs.maxclientslimit * sizeof(client_t));
...
void Host_WriteConfiguration(void) {
  ...
  if (host_initialized && !isDedicated && !host_parms->errstate) {
    ...
    fprintf(f, "+mlook\n"); // always enable mouse look on config, can be
                            // overriden by -mlook in autoexec.cfg
    // johnfitz
...
newmap contains the path of the map that is about to be loaded. If it matches
the last one and keepbmodelcache is set, brush models are not freed. If it's
NULL, brush models are not freed (pending a later call to Mod_ClearBModelCaches
once the map name is known)
================
*/
...
bool Host_FilterTime(float time) {
  ...
  if (host_maxfps.value) {
    ...
    // Check if we still have more than 2ms till next frame and if so wait for
    // "1ms" E.g. Windows is not a real time OS and the sleeps can vary in
    // length even with timeBeginPeriod(1)
...
static void CL_LoadCSProgs(void) {
  ...
  if (pr_checkextension.value &&
    ...
    PR_SwitchQCVM(&cl.qcvm);
    csqchash = strtoul(Info_GetKey(cl.serverinfo, "*csprogs", versionedname,
                                   sizeof(versionedname)),
                       NULL, 0);
    ...
    if ((PR_LoadProgs(versionedname, false, PROGHEADER_CRC, pr_csqcbuiltins,
                      pr_csqcnumbuiltins) &&
         qcvm->extfuncs.CSQC_DrawHud) ||
        (PR_LoadProgs("csprogs.dat", false, PROGHEADER_CRC, pr_csqcbuiltins,
                      pr_csqcnumbuiltins) &&
         qcvm->extfuncs.CSQC_DrawHud) ||
        (PR_LoadProgs("progs.dat", false, PROGHEADER_CRC, pr_csqcbuiltins,
                      pr_csqcnumbuiltins) &&
         qcvm->extfuncs.CSQC_DrawHud)) {
...
void _Host_Frame(double time) {
  ...
  while ((host_netinterval == 0) || (accumtime >= host_netinterval)) {
    ...
    if (host_netinterval && isDedicated == 0) {
      host_frametime =
          sv.active ? (listening ? q_min(accumtime, 0.017) : host_netinterval)
                    : accumtime;
