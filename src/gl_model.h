...
typedef struct msurface_s {
  ...
  uint32_t styles_bitmap; // bitmap of styles used (16..64 OR-folded into
                          // bits 16..31)
...
typedef struct mleaf_s {
  ...
  // leaf specific
  int nummarksurfaces;
  int combined_deps; // contains index into brush_deps_data[] with used warp and
                     // lightmap textures
...
#define MOD_EMITFORWARDS                                                       \
  4096 // particle effect is emitted forwards, rather than downwards. why down?
       // good question.
// spike
...
typedef struct qmodel_s {
  ...
  bool primed; // if true, this model is placed in the lightmap and has its
               // vertex buffers and accel structures built
...
#ifdef PSET_SCRIPT
  int emiteffect;  // spike -- this effect is emitted per-frame by entities with
                   // this model
  ...
  int combined_deps; // contains index into brush_deps_data[] with used warp and
                     // lightmap textures
  int used_specials; // contains SURF_DRAWSKY, SURF_DRAWTURB, SURF_DRAWWATER,
                     // SURF_DRAWLAVA, SURF_DRAWSLIME, SURF_DRAWTELE flags if
                     // used by any surf

  int *water_surfs; // worldmodel only: list of surface indices with
                    // SURF_DRAWTURB flag of transparent types
  int used_water_surfs;
  int water_surfs_specials; // which surfaces are in water_surfs
                            // (SURF_DRAWWATER, SURF_DRAWLAVA, SURF_DRAWSLIME,
                            // SURF_DRAWTELE) to track transparency changes
  ...
  bool md5_prio; // if true, the MD5 model has at least as much path priority as
                 // the MDL model
