...
cvar::CVar cmdline = {
    "cmdline", "", cvar::ROM /*|CVAR_SERVERINFO*/}; /* sending cmdline upon
...
int32_t vsnprintf(char *str, size_t size, const char *format, va_list args)
    __attribute__((__format__(printf, 3, 0)));
int32_t vsnprintf(char *str, size_t size, const char *format, va_list args) {
  int ret = std::vsnprintf(str, size, format, args);
...
int32_t snprintf(char *str, size_t size, const char *format, ...) {
  ...
  int32_t ret = vsnprintf(str, size, format, argptr);
  va_end(argptr);
