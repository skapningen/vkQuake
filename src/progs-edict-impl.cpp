...
CVar nomonsters = {"nomonsters", "0", cvar::NONE};
CVar gamecfg = {"gamecfg", "0", cvar::NONE};
CVar scratch1 = {"scratch1", "0", cvar::NONE};
CVar scratch2 = {"scratch2", "0", cvar::NONE};
CVar scratch3 = {"scratch3", "0", cvar::NONE};
CVar scratch4 = {"scratch4", "0", cvar::NONE};
CVar savedgamecfg = {"savedgamecfg", "0", cvar::ARCHIVE};
CVar saved1 = {"saved1", "0", cvar::ARCHIVE};
CVar saved2 = {"saved2", "0", cvar::ARCHIVE};
CVar saved3 = {"saved3", "0", cvar::ARCHIVE};
CVar saved4 = {"saved4", "0", cvar::ARCHIVE};
...
=================
*/
progs::EntityDict *ED_Alloc(void) {
  progs::EntityDict *e = qcvm->free_edicts_head;
...
=================
*/
void ED_Free(progs::EntityDict *ed) {
  if (ed->free) {
...
Used at load time to place edicts at a specifit spot, and to trim
qcvm->num_edicts
=================
*/
void ED_RemoveFromFreeList(progs::EntityDict *ed) {
  assert(ed->free);
...
dfunction_t *ED_FindFunction(const char *fn_name) {
  dfunction_t **func_ptr =
      HashMap_Lookup(dfunction_t *, qcvm->function_map, &fn_name);
  if (func_ptr)
    return *func_ptr;
  return NULL;
}
...
============
*/
eval_t *GetEdictFieldValue(progs::EntityDict *ed, int fldofs) {
  if (fldofs < 0)
    return NULL;
...
=============
*/
void ED_Print(progs::EntityDict *ed) {
  ddef_t *d;
  int *v;
  int i, j, l;
  const char *name;
...
=============
*/
void ED_Write(FILE *f, progs::EntityDict *ed) {
  ddef_t *d;
  int *v;
  int i;
  int type;
  ...
  for (i = 1; i < qcvm->progs->numfielddefs; i++) {
    ...
    assert(
        !!(type & DEF_SAVEGLOBAL) ==
        (strlen(PR_GetString(d->s_name)) > 1 &&
         PR_GetString(d->s_name)[strlen(PR_GetString(d->s_name)) - 2] == '_'));
    ...
    // if the value is still all 0, skip the field
    assert(type < 8 && ((type == ev_vector && type_size[type] == 3) ||
                        (type != ev_vector && type_size[type] == 1)));
...
qboolean ED_ParseEpair(void *base, ddef_t *key, const char *s, qboolean zoned) {
  ...
  case ev_entity:
    if (!strncmp(s, "entity ", 7)) // Spike: putentityfieldstring/etc should be
                                   // able to cope with etos's weirdness.
      s += 7;
...
====================
*/
const char *ED_ParseEdict(const char *data, progs::EntityDict *ent) {
  ddef_t *key;
  char keyname[256];
...
void clear_progs(qcvm_t *vm) {
  ...
  Mem_Free(qcvm->progs); // spike -- pr_progs switched to use malloc (so menuqc
                         // doesn't end up stuck on the early hunk nor wiped on
                         // every map change)
...
static void PR_MergeEngineFieldDefs(void) {
  struct {
    ...
    int newidx;
      ...
      // these later.
      // this is useful for fields that should be defined for mappers which are
      // not defined by the mod.
      // future note: mutators will need to edit the mutator's globaldefs table
      // too. remember to handle vectors and their 3 globals too.
      ...
      if (extrafields[j].newidx >= qcvm->progs->entityfields &&
        ...
        if (extrafields[j].type ==
          ...
          for (a = 0; a < 3; a++) {
            ...
            qcvm->fielddefs[qcvm->progs->numfielddefs].type =
                ev_float | DEF_SAVEGLOBAL;
            const char *fielddef_name =
                va("%s_%c", extrafields[j].fname, 'x' + a);
            qcvm->fielddefs[qcvm->progs->numfielddefs].s_name =
                ED_NewString(fielddef_name);
            const ddef_t *def_ptr_v =
                &qcvm->fielddefs[qcvm->progs->numfielddefs];
...
Disables Quake 2021 release effects flags when not present in progs.dat to avoid
conflicts (e.g. Arcane Dimensions uses bit 32 for its explosions, same as
EF_QEX_PENTALIGHT)
===============
*/
...
qboolean PR_LoadProgs(const char *filename, qboolean fatal,
                      unsigned int needcrc, builtin_t *builtins,
                      size_t numbuiltins) {
  int i;
  ...
  qcvm->function_map =
      HashMap_Create(const char *, dfunction_t *, &HashStr, &HashStrCmp);
  ...
  qcvm->globaldefs_map =
      HashMap_Create(const char *, ddef_t *, &HashStr, &HashStrCmp);
  ...
  qcvm->fielddefs_map =
      HashMap_Create(const char *, ddef_t *, &HashStr, &HashStrCmp);
  HashMap_Reserve(qcvm->fielddefs_map,
                  qcvm->progs->numfielddefs +
                      11); // up to 7 scalar + 1 vector engine autofields
...
#undef QCEXTFIELD
  ...
  qcvm->edict_size =
      qcvm->progs->entityfields * 4 + sizeof(progs::EntityDict) - sizeof(entvars_t);
...
progs::EntityDict *EDICT_NUM(int n) {
  if (n < 0 || n >= qcvm->max_edicts)
    Host_Error("EDICT_NUM: bad number %i", n);
  return (progs::EntityDict *)((byte *)qcvm->edicts + (n)*qcvm->edict_size);
}

int NUM_FOR_EDICT(progs::EntityDict *e) {
...
static void PR_AllocStringSlots(void) {
  ...
  qcvm->knownstrings = (const char **)Mem_Realloc(
      (void *)qcvm->knownstrings, qcvm->maxknownstrings * sizeof(char *));
  qcvm->knownstringsowned =
      (qboolean *)Mem_Realloc((void *)qcvm->knownstringsowned,
                              qcvm->maxknownstrings * sizeof(qboolean));
}
