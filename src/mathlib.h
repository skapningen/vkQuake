...
void VectorAngles(const vec3_t forward, float *up,
                  vec3_t angles); // johnfitz, spike(up is optional)
...
bool IsAxisAlignedDeg(vec3_t angle);
bool IsOriginWithinMinMax(vec3_t origin, vec3_t mins, vec3_t maxs);
