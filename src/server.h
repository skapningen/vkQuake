...
typedef struct client_s {
  ...
  bool spawned; // false = don't send datagrams (set when client acked the first
                // entities)
  ...
  unsigned int
      resendstatsnum[MAX_CL_STATS / 32]; // the stats which need to be resent.
  unsigned int
      resendstatsstr[MAX_CL_STATS / 32]; // the stats which need to be resent.
...
#define SENDFLAG_PRESENT                                                       \
  0x80000000u // tracks that we previously sent one of these ents (resulting in
              // a remove if the ent gets remove()d).
#define SENDFLAG_REMOVE                                                        \
  0x40000000u // for packetloss to signal that we need to resend a remove.
#define SENDFLAG_USABLE                                                        \
  0x00ffffffu // SendFlags bits that the qc is actually able to use (don't get
              // confused if the mod uses SendFlags=-1).
  struct deltaframe_s { // quick overview of how this stuff actually works:
    // when the server notices a gap in the ack sequence, we walk through the
    // dropped frames and reflag everything that was dropped. if the server
    // isn't tracking enough frames, then we just treat those as dropped; small
    // note: when an entity is new, it re-flags itself as new for the next
    // packet too, this reduces the immediate impact of packetloss on new
    // entities. reflagged state includes stats updates, entity updates, and
    // entity removes.
    ...
    int numents; // doesn't contain an entry for every entity, just ones that
                 // were sent this frame. no 0 bits
    int maxents;
  } *frames;
...
#define MSG_EXT_MULTICAST                                                      \
  4 // temporary buffer that can be splurged more reliably / with more control.
#define MSG_EXT_ENTITY                                                         \
  5 // for csqc networking. we don't actually support this. I'm just defining it
    // for completeness.
