...
constexpr size_t STRINGTEMP_BUFFERS = 1024;
// #define STRINGTEMP_BUFFERS 16
// #define STRINGTEMP_BUFFERS 1024
constexpr size_t STRINGTEMP_LENGTH = 1024;
  ...
  if (sv.num_ambients == sv.max_ambients) {
    ...
    struct ambientsound_s *n =
        (nm * sizeof(*n) < sv.max_ambients * sizeof(*n))
            ? NULL
            : Mem_Realloc(sv.ambientsounds, nm * sizeof(*n));
    if (!n)
...
static void PF_vtos(void) {
  ...
  s = PR_GetTempString();
  q_snprintf(s, STRINGTEMP_LENGTH, "'%5.1f %5.1f %5.1f'",
...
int SV_Precache_Sound(const char *s) { // must be a persistent string.
  int i;
  ...
  for (i = 0; i < MAX_SOUNDS; i++) {
    if (!sv.sound_precache[i]) {
      ...
      {
        Con_Warning("PF_precache_sound(\"%s\"): Precache should only be done "
                    "in spawn functions\n",
                    s);
...
static void PF_sv_precache_model(void) {
  ...
  for (i = 0; i < MAX_MODELS; i++) {
    if (!sv.model_precache[i]) {
      if (sv.state != ss_loading) {
        Con_Warning("PF_precache_model(\"%s\"): Precache should only be done "
                    "in spawn functions\n",
                    s);
        // let existing clients know about it
        MSG_WriteByte(&sv.reliable_datagram, svcdp_precache);
...
static void PF_coredump(void) { Cbuf_AddText("edicts\n"); }
...
static void PF_sv_WriteEntity(void) {
  extern unsigned int
      sv_protocol_pext2; // spike -- this ought to be client-specific, but we
                         // can't cope with that, so just live with the problems
                         // when ents>32768 (which QS doesn't support anyway)
...
static void PF_sv_makestatic(void) {
  ...
  if (sv.num_statics == sv.max_statics) {
    ...
    entity_state_t *n = (nm * sizeof(*n) < sv.max_statics * sizeof(*n))
                            ? NULL
                            : Mem_Realloc(sv.static_entities, nm * sizeof(*n));
    if (!n)
...
==============
*/
void PF_sv_finalefinished(void) { G_FLOAT(OFS_RETURN) = 0; }
void PF_sv_CheckPlayerEXFlags(void) { G_FLOAT(OFS_RETURN) = 0; }
void PF_sv_walkpathtogoal(void) { G_FLOAT(OFS_RETURN) = 0; /* PATH_ERROR */ }
