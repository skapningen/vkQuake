...
int PR_Markup_Parse(struct markup_s *mu) {
  ...
  while ((c = *mu->txt)) {
    if (c == '^' &&
        pr_checkextension.value) { // parse markup like FTE/DP might.
      switch (mu->txt[1]) {
...
static void
PF_ext_vectoangles(void) { // alternative version of the original builtin, that
                           // can deal with roll angles too, by accepting an
                           // optional second argument for 'up'.
  float *value1, *up;
  ...
  G_VECTOR(OFS_RETURN)
  [PITCH] *= -1; // this builtin is for use with models. models have an inverted
...
static void PR_UnzoneAll(void) { // called to clean up all zoned strings.
...
static void PF_strreplace(void) {
  ...
  if (searchlen) {
    while (*subject &&
           result < resultbuf + STRINGTEMP_LENGTH - replacelen - 2) {
...
static void PF_strireplace(void) {
  ...
  if (searchlen) {
    while (*subject &&
           result < resultbuf + sizeof(resultbuf) - replacelen - 2) {
        ...
        *f++ = 0;
        ...
        case 'c':
          ...
          /*							else
             ...
             0)
             // not set precision = end - o - 1; o += u8_strpad(o, end - o, buf,
...
static void PF_stoi(void) { G_INT(OFS_RETURN) = atoi(G_STRING(OFS_PARM0)); }
...
// collision stuff
static void PF_tracebox(void) { // alternative version of traceline that just
                                // passes on two extra args. trivial really.
...
static void PF_TraceToss(void) {
       ...
       i++) // LordHavoc: sanity check; never trace more than 10 seconds
  {
    ...
    trace = SV_Move(origin, tossent->v.mins, tossent->v.maxs, end, MOVE_NORMAL,
                    tossent);
    ...
    if (VectorLength(velocity) > sv_maxvelocity.value) {
      ...
      VectorScale(velocity, sv_maxvelocity.value / VectorLength(velocity),
                  velocity);
    }
  }
...
static void PF_sv_setmodelindex(void) {
  ...
  e->v.model =
      (newidx < MAX_MODELS) ? PR_SetEngineString(sv.model_precache[newidx]) : 0;
...
static void
PF_getsurfacenumtriangles(void) { // for q3bsp compat (which this engine doesn't
                                  // support, so its fairly simple)
  edict_t *ed = G_EDICT(OFS_PARM0);
  unsigned int surfidx = G_FLOAT(OFS_PARM1);
  qmodel_t *mod = qcvm->GetModel(ed->v.modelindex);
...
static void
PF_getsurfacetriangle(void) { // for q3bsp compat (which this engine doesn't
                              // support, so its fairly simple)
  edict_t *ed = G_EDICT(OFS_PARM0);
  unsigned int surfidx = G_FLOAT(OFS_PARM1);
...
static float getsurface_clippointpoly(qmodel_t *model, msurface_t *surf,
                                      vec3_t point, vec3_t bestcpoint,
                                      float bestdist, float *distsquare) {
  int e, edge;
  ...
  *distsquare = dist * dist;
  ...
  if (*distsquare < bestdist) { // within a specific range
    ...
    for (e = surf->firstedge + surf->numedges; e > surf->firstedge; edge++) {
      ...
      dist =
          DotProduct(v1->position, edgenormal) - DotProduct(cpoint, edgenormal);
      if (dist < 0)
...
#define NEARSURFACE_CACHEHITDISTSQUARED                                        \
  ((NEARSURFACE_CACHEDIST - NEARSURFACE_MAXDIST) *                             \
   (NEARSURFACE_CACHEDIST - NEARSURFACE_MAXDIST))
...
static void PF_getsurfacenearpoint(void) {
  ...
  if (cacheable && nearsurface_cache_valid) {
    ...
    if (DotProduct(cache_distance, cache_distance) <
        NEARSURFACE_CACHEHITDISTSQUARED) // cached list can be used
    {
      for (i = 0; i < nearsurface_cache_entries; i++) {
        dist = getsurface_clippointpoly(model, surf + nearsurface_cache[i],
                                        point, cpoint, bestdist, &distsquare);
        if (dist < bestdist) {
          bestdist = dist;
  ...
  if (!cached) {
    ...
    for (i = 0; i < model->nummodelsurfaces; i++, surf++) {
      ...
      if (cacheable &&
          distsquare < NEARSURFACE_CACHEDIST * NEARSURFACE_CACHEDIST) {
...
static void PF_getsurfacepointattribute(void) {
  ...
  if (mod && mod->type == mod_brush && !mod->needload &&
    ...
    case 2: // t dir
    {
      // figure out how similar to the normal it is, and negate any influence,
      // so that its perpendicular
      float sc =
          -DotProduct(fa->plane->normal, fa->texinfo->vecs[attribute - 1]);
      VectorMA(fa->texinfo->vecs[attribute - 1], sc, fa->plane->normal,
               G_VECTOR(OFS_RETURN));
    ...
    case 3: // normal
      ...
      break;
    case 4: // st coord
      G_FLOAT(OFS_RETURN + 0) = (DotProduct(v->position, fa->texinfo->vecs[0]) +
                                 fa->texinfo->vecs[0][3]) /
                                fa->texinfo->texture->width;
      G_FLOAT(OFS_RETURN + 1) = (DotProduct(v->position, fa->texinfo->vecs[1]) +
                                 fa->texinfo->vecs[1][3]) /
    ...
    case 5: // lmst coord, not actually very useful
      G_FLOAT(OFS_RETURN + 0) =
          (DotProduct(v->position, fa->texinfo->vecs[0]) +
           fa->texinfo->vecs[0][3] - fa->texturemins[0] + (fa->light_s + .5)) /
          LMBLOCK_WIDTH;
      G_FLOAT(OFS_RETURN + 1) =
          (DotProduct(v->position, fa->texinfo->vecs[1]) +
           fa->texinfo->vecs[1][3] - fa->texturemins[1] + (fa->light_t + .5)) /
          LMBLOCK_HEIGHT;
      G_FLOAT(OFS_RETURN + 2) = 0;
      break;
    case 6: // colour
...
static void PF_sv_getlight(void) {
  ...
  // won't either.
  vec3_t lightcolor;
  G_FLOAT(OFS_RETURN + 0) = G_FLOAT(OFS_RETURN + 1) = G_FLOAT(OFS_RETURN + 2) =
      R_LightPoint(point, 0.f, &lc, &lightcolor) / 255.0;
    ...
    for (i = svs.maxclients; i-- > 0;) {
      if (!svs.clients[i].active) {
        svs.clients[i].netconnection =
            NULL; // botclients have no net connection, obviously.
        SV_ConnectClient(i);
...
static void PF_dropclient(void) {
  ...
  unsigned int i = NUM_FOR_EDICT(ed) - 1;
  if (i < (unsigned int)svs.maxclients &&
      svs.clients[i].active) { // FIXME: should really set a flag or something,
...
static void PF_cvar_string(void) {
  ...
  if (var && var->string) {
    ...
    // this would result in leaks/exploits/slowdowns if the qc spams calls to
    // cvar_string+changes. so keep performance consistent, even if this is
    // going to be slower.
    ...
    G_INT(OFS_RETURN) = PR_SetEngineString(temp);
                 ...
                 name,
                 "game")) { // game looks like a cvar in most other respects
                            // (and is a cvar in fte). let cvar_string work on
                            // it as a way to find out the current gamedir.
    char *temp = PR_GetTempString();
...
static void
PF_cvar_description(void) { // quakespasm does not support cvar descriptions. we
                            // provide this stub to avoid crashes.
  G_INT(OFS_RETURN) = 0;
}
...
static void PF_WriteString2(
    void) { // writes a string without the null. a poor-man's strcat.
  const char *string = G_STRING(OFS_PARM0);
  SZ_Write(WriteDest(), string, strlen(string));
}
static void PF_WriteFloat(void) { // curiously, this was missing in vanilla.
  MSG_WriteFloat(WriteDest(), G_FLOAT(OFS_PARM0));
}
static void
PF_sv_te_blooddp(void) { // blood is common enough that we should emulate it for
                         // when engines do actually support it.
...
static void PF_sv_te_bloodqw(void) { // qw tried to strip a lot.
  float *org = G_VECTOR(OFS_PARM0);
...
static void
PF_sv_te_lightningblood(void) { // a qw builtin, to replace particle.
  float *org = G_VECTOR(OFS_PARM0);
  vec3_t dir = {0, 0, -100};
  float color = 20;
  float count = 225;
...
static void PF_cl_te_lightning1(void) {
  ...
  CL_UpdateBeam(Mod_ForName("progs/bolt.mdl", true), "TE_LIGHTNING1",
                "TE_LIGHTNING1_END", -NUM_FOR_EDICT(ed), start, end);
}
...
static void PF_cl_te_lightning2(void) {
  ...
  CL_UpdateBeam(Mod_ForName("progs/bolt2.mdl", true), "TE_LIGHTNING2",
                "TE_LIGHTNING2_END", -NUM_FOR_EDICT(ed), start, end);
}
...
static void PF_cl_te_lightning3(void) {
  ...
  CL_UpdateBeam(Mod_ForName("progs/bolt3.mdl", true), "TE_LIGHTNING3",
                "TE_LIGHTNING3_END", -NUM_FOR_EDICT(ed), start, end);
}
...
static void PF_cl_te_beam(void) {
  ...
  CL_UpdateBeam(Mod_ForName("progs/beam.mdl", true), "TE_BEAM", "TE_BEAM_END",
                -NUM_FOR_EDICT(ed), start, end);
}
#ifdef PSET_SCRIPT
...
static void PF_cl_localsound(void) {
  ...
  S_StartSound(cl.viewentity, channel, S_PrecacheSound(sample), vec3_origin,
               volume, 0);
}
// file stuff

static void PF_whichpack(void) {
  const char *fname = G_STRING(
      OFS_PARM0); // uses native paths, as this isn't actually reading anything.
  unsigned int path_id;
  if (COM_FileExists(fname, &path_id)) {
    // FIXME: quakespasm reports which gamedir the file is in, but paks are
    // hidden. I'm too lazy to rewrite COM_FindFile, so I'm just going to hack
    // something small to get the gamedir, just not the paks
    ...
    if (!strncmp(fname, com_basedir, strlen(com_basedir)))
      ...
      fname =
          "?"; // no idea where this came from. something is screwing with us.
    while (*fname == '/' || *fname == '\\')
...
static void PF_bufstr_set(void) {
  ...
  if (index >= strbuflist[bufno].allocated) {
    ...
    strbuflist[bufno].strings =
        Mem_Realloc(strbuflist[bufno].strings,
...
static int PF_bufstr_add_internal(unsigned int bufno, const char *string,
                                  int appendonend) {
  unsigned int index;
  if (appendonend) {
    // add on end
...
static void PF_buf_cvarlist(void) {
  ...
  for (var = Cvar_FindVarAfter("", CVAR_NONE); var; var = var->next) {
    if (plen && (pwc ? !wildcmp(pattern, var->name)
                     : strncmp(var->name, pattern, plen)))
      continue;
    if (alen && (awc ? wildcmp(antipattern, var->name)
                     : !strncmp(var->name, antipattern, alen)))
      continue;

    PF_bufstr_add_internal(bufno, var->name, true);
  }

  qsort(strbuflist[bufno].strings, strbuflist[bufno].used, sizeof(char *),
        PF_buf_sort_ascending);
}
...
static void PF_getentfldstr(void) {
  ...
  if (fldidx < (unsigned int)qcvm->progs->numfielddefs) {
    char *ret = PR_GetTempString();
    const char *val = PR_UglyValueString(
        qcvm->fielddefs[fldidx].type,
...
static void PF_putentfldstr(void) {
  ...
  if (fldidx < (unsigned int)qcvm->progs->numfielddefs)
    G_FLOAT(OFS_RETURN) =
        ED_ParseEpair((void *)&ent->v, qcvm->fielddefs + fldidx, value, true);
  else
...
static void PF_infokey_internal(qboolean returnfloat) {
  ...
  if (!ent) {
    if (!strcmp(key, "*version")) {
      ...
      r = NULL;
  } else if (ent <= (unsigned int)svs.maxclients &&
             svs.clients[ent - 1].active) {
    ...
    else if (!strcmp(key, "team")) // nq doesn't really do teams. qw does
                                   // though. yay compat?
...
static void PF_infokey_s(void) { PF_infokey_internal(false); }
static void PF_infokey_f(void) { PF_infokey_internal(true); }
...
static void PF_multicast_internal(qboolean reliable, byte *pvs,
  ...
  if (!pvs) {
    if (!requireext2)
      ...
      for (i = 0; i < (unsigned int)svs.maxclients; i++) {
        ...
        if (!(svs.clients[i].protocol_pext2 & requireext2))
          continue;
        SZ_Write(
            (reliable ? &svs.clients[i].message : &svs.clients[i].datagram),
            sv.multicast.data, sv.multicast.cursize);
      }
    }
  } else {
    for (i = 0; i < (unsigned int)svs.maxclients; i++) {
      ...
      playerleaf =
          Mod_PointInLeaf(svs.clients[i].edict->v.origin, qcvm->worldmodel);
...
static void SV_Multicast(multicast_t to, float *org, int msg_entity,
  ...
  case MULTICAST_PHS_U:
    // we don't support phs, that would require lots of pvs
    // decompression+merging stuff, and many q1bsps have a LOT of leafs.
    PF_multicast_internal(to == MULTICAST_PHS_R,
                          NULL /*Mod_LeafPHS(Mod_PointInLeaf(org,
                                  qcvm->worldmodel), qcvm->worldmodel)*/
                          ,
                          requireext2);
    break;
  ...
  case MULTICAST_PVS_U:
    PF_multicast_internal(
        to == MULTICAST_PVS_R,
        Mod_LeafPVS(Mod_PointInLeaf(org, qcvm->worldmodel), qcvm->worldmodel),
        requireext2);
    break;
...
static void PF_multicast(void) {
  ...
  SV_Multicast(to, org,
               NUM_FOR_EDICT(PROG_TO_EDICT(pr_global_struct->msg_entity)), 0);
}
  ...
  for (i = 1; i < MAX_PARTICLETYPES; i++) {
    if (!sv.particle_precache[i]) {
      if (sv.state != ss_loading) {
        ...
        SV_Multicast(MULTICAST_ALL_R, NULL, 0,
                     PEXT2_REPLACEMENTDELTAS); // FIXME
      }

      sv.particle_precache[i] =
          q_strdup(s); // weirdness to avoid issues with tempstrings
      return i;
    }
  }
  return 0;
    ...
    if (sv.particle_precache[i] && !strcmp(sv.particle_precache[i], s)) {
      if (sv.state != ss_loading && !pr_checkextension.value) {
        if (pr_ext_warned_particleeffectnum++ < 3)
          Con_Warning("PF_sv_particleeffectnum(%s): Precache should only be "
...
int PF_CL_GetParticle(
    int idx) { // negatives are csqc-originated particles, positives are
               // ssqc-originated, for consistency allowing networking of
               // particles as identifiers
  if (!idx)
    return P_INVALID;
  if (idx < 0) {
    idx = -idx;
...
static void PF_cl_trailparticles(void) {
  ...
  if ((unsigned int)G_INT(OFS_PARM1) >=
      MAX_EDICTS *
          (unsigned int)qcvm
              ->edict_size) { /*DP gets this wrong, lets try to be compatible*/
    ent = G_EDICT(OFS_PARM0);
    efnum = G_FLOAT(OFS_PARM1);
  } else {
    efnum = G_FLOAT(OFS_PARM0);
  ...
  PScript_ParticleTrail(start, end, efnum, host_frametime, -NUM_FOR_EDICT(ent),
                        NULL, NULL /*&ent->trailstate*/);
}
...
static qpic_t *DrawQC_CachePic(
  ...
  if (flags & PICFLAG_WRAP)
    texflags &= ~TEXPREF_PAD; // don't allow padding if its going to need to
                              // wrap (even if we don't enable clamp-to-edge
                              // normally). I just hope we have npot support.
...
static void DrawQC_CharacterQuad(cb_context_t *cbx, float x, float y, int num,
                                 float w, float h, float *rgb, float alpha) {
  float size = 0.0625;
  ...
  basicvertex_t *vertices = (basicvertex_t *)R_VertexAllocate(
      6 * sizeof(basicvertex_t), &buffer, &buffer_offset);
  ...
  vulkan_globals.vk_cmd_bind_vertex_buffers(cbx->cb, 0, 1, &buffer,
                                            &buffer_offset);
  if (alpha_blend)
    R_BindPipeline(cbx, VK_PIPELINE_BIND_POINT_GRAPHICS,
                   vulkan_globals.basic_blend_pipeline[cbx->render_pass_index]);
  else
    R_BindPipeline(
  ...
  vulkan_globals.vk_cmd_bind_descriptor_sets(
      ...
      vulkan_globals.basic_pipeline_layout.handle, 0, 1,
      &char_texture->descriptor_set, 0, NULL);
...
static void PF_cl_drawcharacter(void) {
  ...
  DrawQC_CharacterQuad(vulkan_globals.secondary_cb_contexts[SCBX_GUI], pos[0],
                       pos[1], charcode, size[0], size[1], rgb, alpha);
}
...
static void PF_cl_drawrawstring(void) {
  ...
  while ((c = *text++)) {
    DrawQC_CharacterQuad(vulkan_globals.secondary_cb_contexts[SCBX_GUI], x,
                         pos[1], c, size[0], size[1], rgb, alpha);
    x += size[0];
  }
}
...
static void PF_cl_drawsetclip(void) {
  ...
  vkCmdSetScissor(vulkan_globals.secondary_cb_contexts[SCBX_GUI][0].cb, 0, 1,
                  &render_area);
}
...
static void PF_cl_drawpic(void) {
  ...
  if (pic)
    Draw_SubPic(vulkan_globals.secondary_cb_contexts[SCBX_GUI], pos[0], pos[1],
                size[0], size[1], pic, 0, 0, 1, 1, rgb, alpha);
}
...
static void PF_cl_drawsubpic(void) {
  ...
  if (pic)
    Draw_SubPic(vulkan_globals.secondary_cb_contexts[SCBX_GUI], pos[0], pos[1],
                size[0], size[1], pic, srcpos[0], srcpos[1], srcsize[0],
                srcsize[1], rgb, alpha);
}
...
static void PF_cl_drawfill(void) {
  ...
  R_BindPipeline(
      cbx, VK_PIPELINE_BIND_POINT_GRAPHICS,
      vulkan_globals.basic_notex_blend_pipeline[cbx->render_pass_index]);
  vulkan_globals.vk_cmd_bind_descriptor_sets(
      cbx->cb, VK_PIPELINE_BIND_POINT_GRAPHICS,
      vulkan_globals.basic_pipeline_layout.handle, 0, 1,
      &char_texture->descriptor_set, 0, NULL);
...
static void PF_checkpvs(void) {
  ...
  byte *pvs = Mod_LeafPVS(
      leaf, qcvm->worldmodel); // johnfitz -- worldmodel as a parameter
  unsigned int i;
...
//          engines that switched to unsigned types to crash due to an
//          underflow. we do some sneaky hacks to avoid changes to the vm...
//          because we're evil.
// 0-199: free for all.
...
qboolean PR_Can_Particles(unsigned int prot, unsigned int pext1,
                          unsigned int pext2) {
...
qboolean PR_Can_Ent_Alpha(unsigned int prot, unsigned int pext1,
                          unsigned int pext2) {
...
qboolean PR_Can_Ent_ColorMod(unsigned int prot, unsigned int pext1,
                             unsigned int pext2) {
...
qboolean PR_Can_Ent_Scale(unsigned int prot, unsigned int pext1,
                          unsigned int pext2) {
...
static struct {
  const char *name;
  qboolean (*checkextsupported)(unsigned int prot, unsigned int pext1,
                                unsigned int pext2);
} qcextensions[] = {
    {"DP_CON_SET"},
    {"DP_CON_SETA"},
...
//CONTENTS_SOLID to CONTENTS_FOO, allowing you to swim in moving ents without qc
//hacks,
//							   // as well as
  ...
  for (i = 0; i < countof(qcextensions); i++) {
    if (!strcmp(extname, qcextensions[i].name)) {
      if (qcextensions[i].checkextsupported) {
        ...
        if (sv.active || qcvm == &sv.qcvm) { // server or client+server
          prot = sv_protocol;
          pext1 = sv_protocol_pext1;
          pext2 = sv_protocol_pext2;

          // if the server seems to be set up for singleplayer then filter by
          // client settings. otherwise just assume the best.
          if (!isDedicated && svs.maxclients == 1 && cl_nopext.value)
            pext1 = pext2 = 0;
        } else if (cls.state == ca_connected) { // client only (or demo)
          prot = cl.protocol;
    ...
    if (binum > 0 && binum < qcvm->numbuiltins) {
      if (qcvm->builtins[binum] == PF_Fixme) {
        ...
        for (i = 0;
             ...
             i++) {
          if (extensionbuiltins[i].number ==
              binum) { // but it will be defined if its actually executed.
            if (extensionbuiltins[i].desc &&
                !strncmp(extensionbuiltins[i].desc, "stub.", 5))
              G_FLOAT(OFS_RETURN) =
                  false; // pretend it won't work if it probably won't be useful
        ...
        G_FLOAT(OFS_RETURN) =
            true; // its defined, within the sane range, mapped, everything. all
                  // looks good. we should probably go through the available
                  // builtins and validate that the qc's name matches what would
                  // be expected this is really intended more for builtins
                  // defined as #0 though, in such cases, mismatched assumptions
                  // are impossible.
      }
    } else
      G_FLOAT(OFS_RETURN) =
          false; // not a valid builtin (#0 builtins get remapped at load, even
                 // if the builtin is activated then)
  } else {       // not valid somehow.
    G_FLOAT(OFS_RETURN) = false;
  }
}
  ...
  if ((unsigned int)glob->function < (unsigned int)qcvm->progs->numfunctions) {
    ...
    if (binum >= 0) {
      ...
      for (i = 0; i < sizeof(extensionbuiltins) / sizeof(extensionbuiltins[0]);
        ...
        if (num == binum) { // set it up so we're faster next time
          builtin_t bi = NULL;
          if (qcvm == &sv.qcvm)
...
#define QCEXTFUNC(n, t) qcvm->extfuncs.n = PR_FindExtFunction(#n);
  ...
  // replace standard builtins with new replacement extended ones and
  // selectively populate references to module-specific entrypoints.
  if (qcvm == &cl.qcvm) { // csqc
    for (i = 0; i < sizeof(extensionbuiltins) / sizeof(extensionbuiltins[0]);
         i++) {
      int num = (extensionbuiltins[i].documentednumber);
      if (num && extensionbuiltins[i].csqcfunc &&
          qcvm->builtins[num] != PF_Fixme)
        qcvm->builtins[num] = extensionbuiltins[i].csqcfunc;
    }

    QCEXTFUNCS_GAME
    QCEXTFUNCS_CS
  } else if (qcvm == &sv.qcvm) { // ssqc
    for (i = 0; i < sizeof(extensionbuiltins) / sizeof(extensionbuiltins[0]);
         i++) {
      int num = (extensionbuiltins[i].documentednumber);
      if (num && extensionbuiltins[i].ssqcfunc &&
          qcvm->builtins[num] != PF_Fixme)
...
#undef QCEXTGLOBAL_VECTOR
  ...
  for (i = 0; i < (unsigned int)qcvm->progs->numfunctions; i++) {
    if (qcvm->functions[i].first_statement == 0 && qcvm->functions[i].s_name &&
        !qcvm->functions[i].parm_start && !qcvm->functions[i].locals) {
      const char *name = PR_GetString(qcvm->functions[i].s_name);
      for (j = 0; j < sizeof(extensionbuiltins) / sizeof(extensionbuiltins[0]);
           j++) {
        if (!strcmp(extensionbuiltins[j].name, name)) { // okay, map it
...
void PR_DumpPlatform_f(void) {
  ...
  fprintf(f, "\n\n//This file only supports csqc, so including this file in "
             "some other situation is a user error\n"
  ...
  if (targs & CS) {
    fprintf(f,
            ...
            "#ifndef CSQC_SIMPLE\n" // quakespasm's csqc implementation is
                                    // simplified, and can do huds+menus, but
                                    // that's about it.
            "#define CSQC_SIMPLE\n"
            "#endif\n"
            "#endif\n");
  } else if (targs & SS) {
    fprintf(f, "#if !defined(CSQC) && !defined(SSQC) && !defined(MENU)\n"
               "#define SSQC\n"
               "#endif\n");
  } else if (targs & MN) {
    fprintf(f, "#if !defined(CSQC) && !defined(SSQC) && !defined(MENU)\n"
  ...
  if (targs & (SS | CS)) // qss uses the same system defs for both ssqc and
                         // csqc, as a simplification.
  { // I really hope that fteqcc's unused variable logic is up to scratch
    ...
    fprintf(f,
            ...
            "found_secrets,killed_monsters,parm1, parm2, parm3, parm4, "
    ...
    fprintf(f, ".float		"
               "weaponframe,currentammo,ammo_shells,ammo_nails,ammo_rockets,"
               "ammo_cells,items,takedamage;\n");
    ...
    fprintf(f, ".float		"
               "flags,colormap,team,max_health,teleport_time,armortype,"
               "armorvalue,waterlevel,watertype,ideal_yaw,yaw_speed;\n");
...
#undef QCEXTGLOBAL_VECTOR
  ...
  if (targs & (CS | SS)) {
    ...
    //		fprintf(f, "const float STAT_FRAGS = 1;			/*
    //unused */\n");
    fprintf(f, "const float STAT_WEAPONMODELI = 2;	/* This is the "
               "modelindex of the current viewmodel (renamed from the original "
               "name 'STAT_WEAPON' due "
               "to confusions). */\n");
    fprintf(f, "const float STAT_AMMO = 3;			/* "
               "player.currentammo */\n");
    ...
    fprintf(
        ...
        "(self.items2<<23). In order to decode this stat properly, you need to "
        "use "
        "getstatbits(STAT_ITEMS,0,23) to read self.items, and "
        "getstatbits(STAT_ITEMS,23,11) to read self.items2 or "
        "getstatbits(STAT_ITEMS,28,4) to "
        "read the visible part of serverflags, whichever is applicable. */\n");
    fprintf(f, "const float STAT_VIEWHEIGHT = 16;	/* player.view_ofs_z "
               "*/\n");
    fprintf(f, "const float STAT_VIEW2 = 20;		/* This stat contains "
               "the number of the entity in the server's .view2 field. */\n");
    fprintf(f, "const float STAT_VIEWZOOM = 21;		/* Scales fov and "
               "sensitivity. Part of DP_VIEWZOOM. */\n");
  ...
  fprintf(f,
          "const float STAT_USER = 32;			/* Custom user stats "
          "start here (lower values are reserved for engine use). */\n");
...
#undef QCEXTFIELD
  ...
  if (targs & SS) {
    ...
    fprintf(
        f, "const float CLIENTTYPE_REAL			= " STRINGIFY(1) ";\n");
    fprintf(f,
            "const float CLIENTTYPE_BOT			= " STRINGIFY(2) ";\n");
  ...
  if (targs & (CS | SS)) {
    ...
    fprintf(f,
            "%svoid(entity e, float chan, string samp, float vol, float atten, "
            "optional float speedpct, optional float flags, optional float "
            "timeofs) "
            "sound = #8;\n",
            conflictprefix);
    ...
    fprintf(f, "void(vector v1, vector v2, float flags, entity ent) traceline "
               "= #16;\n");
    if (targs & SS)
    ...
    if (targs & SS)
      fprintf(f, "%svoid(entity client, string s) stuffcmd = #21;\n",
              conflictprefix);
    fprintf(f,
            "%sentity(vector org, float rad, optional .entity chainfield) "
            "findradius = #22;\n",
            conflictprefix);
    if (targs & SS) {
      fprintf(f,
              "%svoid(string s, optional string s2, optional string s3, "
              "optional string s4, optional string s5, optional string s6, "
              "optional string s7, "
              "optional string s8) bprint = #23;\n",
              conflictprefix);
      fprintf(f,
              "%svoid(entity pl, string s, optional string s2, optional string "
              "s3, optional string s4, optional string s5, optional string s6, "
              "optional "
              "string s7) sprint = #24;\n",
              conflictprefix);
    }
    ...
    fprintf(f,
            "%sfloat(float yaw, float dist, optional float settraceglobals) "
            "walkmove = #32;\n",
            conflictprefix);
    fprintf(f, "float() droptofloor = #34;\n");
    fprintf(f,
            "%svoid(float lightstyle, string stylestring, optional vector rgb) "
            "lightstyle = #35;\n",
            conflictprefix);
    ...
    if (targs & SS)
      fprintf(f, "void(string mapname, optional string newmapstartspot) "
                 "changelevel = #70;\n");
    fprintf(f, "void(string var, string val) cvar_set = #72;\n");
    if (targs & SS)
      fprintf(f, "void(entity ent, string text, optional string text2, "
                 "optional string text3, optional string text4, optional "
                 "string text5, optional "
                 "string text6, optional string text7) centerprint = #73;\n");
    fprintf(f, "void (vector pos, string samp, float vol, float atten) "
               "ambientsound = #74;\n");
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < sizeof(extensionbuiltins) / sizeof(extensionbuiltins[0]);
      ...
      fprintf(f, "%s %s = #%i;", extensionbuiltins[i].typestr,
              extensionbuiltins[i].name, extensionbuiltins[i].documentednumber);
