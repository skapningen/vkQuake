...
qboolean Cmd_Exists(const char *cmd_name) {
  ...
  for (cmd = cmd_functions; cmd; cmd = cmd->next) {
    if (!strcmp(cmd_name, cmd->name)) {
      if (cmd->srctype !=
          src_command) // these commands only exist in certain situations... so
                       // pretend they don't exist here.
        continue;
      return true;
    }
  }
...
void Cmd_ForwardToServer(void) {
  ...
  if (q_strcasecmp(Cmd_Argv(0), "cmd") != 0) {
    ...
    // knows foo because the server is guarenteed a response even if it doesn't
    // understand it, saving a timeout
    if (!strcmp(
            Cmd_Args(),
            "protocols")) { // server asked us for a list of protocol numbers
                            // that we claim to support. this allows cool
                            // servers like fte to autodetect higher limits etc.
      // servers may assume that the client's preferred protocol will be listed
      // first.
      SZ_Print(&cls.message, va("protocols %i %i %i", PROTOCOL_RMQ,
                                PROTOCOL_FITZQUAKE, PROTOCOL_NETQUAKE));
      return;
    }
    if (!strcmp(Cmd_Args(), "pext") &&
        !cl_nopext.value) { // server asked us for a key+value list of the
                            // extensions+attributes we support
