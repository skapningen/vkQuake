...
static int PR_EnterFunction(dfunction_t *f) {
  ...
  if (qcvm->localstack_used + c > LOCALSTACK_SIZE)
    PR_RunError("execute_program: locals stack overflow");
...
static int PR_LeaveFunction(void) {
  ...
  if (qcvm->localstack_used < 0)
    PR_RunError("execute_program: locals stack underflow");
...
void execute_program(func_t fnum) {
  eval_t *ptr;
  dstatement_t *st;
  dfunction_t *f, *newf;
  ...
  if (!fnum || fnum >= (func_t)qcvm->progs->numfunctions) {
    ...
    Host_Error("execute_program: NULL function");
  }
